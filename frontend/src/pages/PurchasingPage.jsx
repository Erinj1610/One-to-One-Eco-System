...
export default function PurchasingPage() {
  ...
  const handleSubmitBatchAllocation = async (e, bypassDocPrompt = false) => {
    ...
    for (const l of selectedLines) {
      const override = batchLineMappings[l.id];
    ...
    const payload = {
      ...
      allocated_by_name: 'Staff',
      notes: batchNotes || `Batch allocated ${itemsPayload.length} items`,
      items: itemsPayload
    };
                    ...
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                        ...
                        {isOrderSelected && (
                          <div style={{ fontSize: '11px', color: isAllocationBlocked ? '#ef4444' : '#10b981', fontWeight: 600 }}>
                            {validCount} of {selectedLines.length} items ready to allocate
                          </div>
                        )}
                      </div>
                      ...
                      {isAllocationBlocked && (
                        <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid #ef4444', borderRadius: '8px', padding: '10px 14px', marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px', color: '#ef4444', fontSize: '11.5px' }}>
                          ...
                          <span><strong>Allocation Blocked:</strong> None of the selected document items belong to this order. Please map at least one line, or select the correct destination order.</span>
                        </div>
                      )}
                        ...
                        <table style={{ width: '100%', fontSize: '11px', borderCollapse: 'collapse' }}>
                          ...
                          <tbody>
                            {linesWithStatus.map(({ line: l, status, matchedItem }) => (
                              <tr key={l.id} style={{ borderBottom: '1px solid var(--border)', opacity: status === 'ignored' ? 0.45 : 1 }}>
                                    ...
                                    {status === 'matched' && (
                                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '6px' }}>
                                        ...
                                        <button 
                                          ...
                                          onClick={() => setBatchLineMappings(prev => ({ ...prev, [l.id]: { type: 'unmatched' } }))}
                                        >
                                          Edit
                                        </button>
                                      </div>
                                      ...
                                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                        <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                          <select 
                                            ...
                                            onChange={(e) => {
                                              if (e.target.value) {
                                                setBatchLineMappings(prev => ({ ...prev, [l.id]: { type: 'item', orderItemId: e.target.value } }));
                                              }
                                            }}
                                            defaultValue=""
                                          >
                                          ...
                                          <button 
                                            ...
                                            onClick={() => setBatchLineMappings(prev => ({ ...prev, [l.id]: { type: 'service' } }))}
                                          >
                                            Service
                                          </button>
                                          <button 
                                            ...
                                            style={{ fontSize: '9.5px', padding: '2px 5px', border: '1px solid var(--border)', whiteSpace: 'nowrap' }}
                                            onClick={() => setBatchLineMappings(prev => ({ ...prev, [l.id]: { type: 'ignore' } }))}
                                          >
                                            Skip
                                          </button>
                                        </div>
                ...
                {(() => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isBlocked ? 'Allocation Blocked (0 Items Mapped)' : `Confirm Batch Allocation (${validCount} items)`}
                    </button>
                  );
                })()}
              </div>
        ...
        <div style={{
          ...
          <div className="card" style={{
            ...
            <p style={{ fontSize: '12.5px', color: 'var(--text-primary)', lineHeight: 1.5, margin: 0 }}>
              Would you like to attach the <strong>Supplier PO / Confirmation</strong> file now? It will automatically be filed in Google Drive under <strong>02 - Supplier POs & Confirmations</strong> for this order.
            </p>
            ...
            <div style={{
              ...
              <input
                type="file"
                id="reminderPurchasingFileInput"
                accept=".pdf,.png,.jpg,.jpeg,.xlsx,.docx"
                onChange={(e) => {
                ...
                style={{ display: 'none' }}
              />
              <label htmlFor="reminderPurchasingFileInput" style={{ cursor: 'pointer', display: 'block' }}>
                {(showNoDocPrompt ? allocFile : batchFile) ? (
                  ...
                  <div>
                    <div style={{ color: '#3b82f6', fontSize: '12.5px', fontWeight: 700, marginBottom: '2px' }}>
                      📁 Click to attach PO document (PDF, PNG, Excel)
                    </div>
                    <div style={{ fontSize: '10.5px', color: 'var(--text-secondary)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '4px' }}>
              ...
              {(showNoDocPrompt ? allocFile : batchFile) ? (
                ...
                <button
                  ...
                  onClick={() => document.getElementById('reminderPurchasingFileInput')?.click()}
                  style={{ fontSize: '12px' }}
                >
                  Choose File...
                </button>
